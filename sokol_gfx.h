...
typedef struct {
    ...
    struct {
        ...
        bool mapped[_SG_WGPU_STAGING_PIPELINE_SIZE];    // true if currently mapped
    } staging;
} _sg_wgpu_ubpool_t;
...
EM_JS(void, sg_wgpu_js_buffer_unmap_range, (WGPUBuffer buf), {
    var buffer_wrapper = WebGPU.mgrBuffer.objects[buf];
    buffer_wrapper.sokol_mapping = null;
});

EM_JS(void, sg_wgpu_js_buffer_copy_range, (WGPUBuffer buf, int dst_offset, const void* src_ptr, int src_size), {
    var buffer_wrapper = WebGPU.mgrBuffer.objects[buf];
    buffer_wrapper.sokol_mapping.set(HEAPU8.subarray(src_ptr, src_ptr + src_size), dst_offset);
});

EM_JS(void, sg_wgpu_js_buffer_copy_rows, (WGPUBuffer buf, int dst_offset, int dst_pitch, const void* src_ptr, int row_size, int row_pitch, int num_rows), {
    var buffer_wrapper = WebGPU.mgrBuffer.objects[buf];
...
_SOKOL_PRIVATE void _sg_wgpu_ubpool_discard(void) {
    ...
    for (int i = 0; i < _sg.wgpu.ub.staging.num; i++) {
        if (_sg.wgpu.ub.staging.buf[i]) {
            ...
            _sg.wgpu.ub.staging.mapped[i] = false;
        }
    }
}
...
_SOKOL_PRIVATE void _sg_wgpu_ubpool_mapped_callback(WGPUBufferMapAsyncStatus status, void* user_data) {
    ...
    SOKOL_ASSERT(!_sg.wgpu.ub.staging.mapped[staging_index]);
    sg_wgpu_js_buffer_map_range(_sg.wgpu.ub.staging.buf[staging_index], 0, (int)_sg.wgpu.ub.num_bytes);
    _sg.wgpu.ub.staging.mapped[staging_index] = true;
}
...
_SOKOL_PRIVATE void _sg_wgpu_ubpool_next_frame(bool first_frame) {
    ...
    for (int i = 0; i < _sg.wgpu.ub.staging.num; i++) {
        if (_sg.wgpu.ub.staging.mapped[i]) {
    ...
    SOKOL_ASSERT(!_sg.wgpu.ub.staging.mapped[cur]);
    sg_wgpu_js_buffer_map_range(_sg.wgpu.ub.staging.buf[cur], 0, (int)_sg.wgpu.ub.num_bytes);
    _sg.wgpu.ub.staging.mapped[cur] = true;
}
...
_SOKOL_PRIVATE void _sg_wgpu_ubpool_flush(void) {
    ...
    const int cur = _sg.wgpu.ub.staging.cur;
    SOKOL_ASSERT(_sg.wgpu.ub.staging.mapped[cur]);
    _sg.wgpu.ub.staging.mapped[cur] = false;
    const WGPUBuffer src_buf = _sg.wgpu.ub.staging.buf[cur];
    sg_wgpu_js_buffer_unmap_range(src_buf);
...
/* helper function to copy image data into a texture via a staging buffer, returns number of
   bytes copied
*/
_SOKOL_PRIVATE int _sg_wgpu_copy_image_data(WGPUBuffer wgpu_buf, int buf_base_offset, _sg_image_t* img, const sg_image_data* data) {
    SOKOL_ASSERT(_sg.wgpu.upload.cmd_enc);
    SOKOL_ASSERT(wgpu_buf);
    SOKOL_ASSERT(img);
    SOKOL_ASSERT(data);
    int buf_offset = buf_base_offset;
    const sg_pixel_format fmt = img->cmn.pixel_format;
    WGPUImageCopyBuffer copy_src;
    memset(&copy_src, 0, sizeof(copy_src));
    copy_src.buffer = wgpu_buf;
    copy_src.layout.offset = (uint64_t)buf_base_offset;
    ...
    for (int mip_index = 0; mip_index < img->cmn.num_mipmaps; mip_index++) {
        ...
        if (src_bytes_per_row == dst_bytes_per_row) {
            ...
            int num_bytes = (int)data->subimage[0][mip_index].size;
            sg_wgpu_js_buffer_copy_range(wgpu_buf, buf_offset, src_base_ptr, num_bytes);
        }
        else {
            // src/dst pitch doesn't match, need to copy row-wise
            int dst_offset = buf_offset;
            const uint8_t* src_ptr = src_base_ptr;
            for (int slice_index = 0; slice_index < num_slices; slice_index++) {
                sg_wgpu_js_buffer_copy_rows(wgpu_buf,
                    dst_offset,
                    dst_bytes_per_row,
                    src_ptr,
                    src_bytes_per_row,
                    src_bytes_per_row,
                    num_rows);
                src_ptr += src_bytes_per_row * num_rows;
                dst_offset += dst_bytes_per_row * num_rows;
            }
        }

        // record the staging copy operations into command encoder
        copy_src.layout.offset = (uint64_t)buf_offset;
        ...
        buf_offset += (uint32_t)(dst_bytes_per_slice * num_slices);
        SOKOL_ASSERT(buf_offset <= (int)_sg.wgpu.upload.num_bytes);
    }
    SOKOL_ASSERT(buf_offset >= buf_base_offset);
    return (buf_offset - buf_base_offset);
}
...
_SOKOL_PRIVATE void _sg_wgpu_staging_discard(void) {
    for (int i = 0; i < _sg.wgpu.upload.staging.num; i++) {
        if (_sg.wgpu.upload.staging.buf[i]) {
            ...
            _sg.wgpu.upload.staging.mapped[i] = false;
        }
    }
}
...
_SOKOL_PRIVATE void _sg_wgpu_staging_mapped_callback(WGPUBufferMapAsyncStatus status, void* user_data) {
    ...
    SOKOL_ASSERT(!_sg.wgpu.upload.staging.mapped[staging_index]);
    sg_wgpu_js_buffer_map_range(_sg.wgpu.upload.staging.buf[staging_index], 0, (int)_sg.wgpu.upload.num_bytes);
    _sg.wgpu.upload.staging.mapped[staging_index] = true;
}
...
_SOKOL_PRIVATE void _sg_wgpu_staging_next_frame(bool first_frame) {
    ...
    for (int i = 0; i < _sg.wgpu.upload.staging.num; i++) {
        if (_sg.wgpu.upload.staging.mapped[i]) {
    ...
    SOKOL_ASSERT(!_sg.wgpu.upload.staging.mapped[cur]);
    sg_wgpu_js_buffer_map_range(_sg.wgpu.upload.staging.buf[cur], 0, (int)_sg.wgpu.upload.num_bytes);
    _sg.wgpu.upload.staging.mapped[cur] = true;
}
...
_SOKOL_PRIVATE uint32_t _sg_wgpu_staging_copy_to_buffer(WGPUBuffer dst_buf, uint32_t dst_buf_offset, const void* data, uint32_t data_num_bytes) {
    ...
    */
    ...
    const int cur = _sg.wgpu.upload.staging.cur;
    SOKOL_ASSERT(_sg.wgpu.upload.staging.mapped[cur]);
    const WGPUBuffer stg_buf = _sg.wgpu.upload.staging.buf[_sg.wgpu.upload.staging.cur];
    SOKOL_ASSERT(stg_buf);
    sg_wgpu_js_buffer_copy_range(stg_buf, (int)_sg.wgpu.upload.offset, data, (int)data_num_bytes);
    wgpuCommandEncoderCopyBufferToBuffer(_sg.wgpu.upload.cmd_enc, stg_buf, _sg.wgpu.upload.offset, dst_buf, dst_buf_offset, copy_num_bytes);
    _sg.wgpu.upload.offset += _sg_roundup_u32(_sg.wgpu.upload.offset + copy_num_bytes, _SG_WGPU_STAGING_ALIGN);
...
_SOKOL_PRIVATE bool _sg_wgpu_staging_copy_to_texture(_sg_image_t* img, const sg_image_data* data) {
    ...
    SOKOL_ASSERT(_sg.wgpu.upload.cmd_enc);
    const uint32_t copy_num_bytes = _sg_wgpu_image_data_buffer_size(img);
    if ((_sg.wgpu.upload.offset + copy_num_bytes) >= _sg.wgpu.upload.num_bytes) {
    ...
    const int cur = _sg.wgpu.upload.staging.cur;
    SOKOL_ASSERT(_sg.wgpu.upload.staging.mapped[cur]);
    const WGPUBuffer stg_buf = _sg.wgpu.upload.staging.buf[cur];
    SOKOL_ASSERT(stg_buf);
    uint32_t bytes_copied = (uint32_t)_sg_wgpu_copy_image_data(stg_buf, (int)_sg.wgpu.upload.offset, img, data);
    SOKOL_ASSERT(bytes_copied == copy_num_bytes); _SOKOL_UNUSED(bytes_copied);
    _sg.wgpu.upload.offset = _sg_roundup_u32(_sg.wgpu.upload.offset + copy_num_bytes, _SG_WGPU_STAGING_ALIGN);
    return true;
}
...
_SOKOL_PRIVATE void _sg_wgpu_staging_unmap(void) {
    ...
    const int cur = _sg.wgpu.upload.staging.cur;
    SOKOL_ASSERT(_sg.wgpu.upload.staging.mapped[cur]);
    _sg.wgpu.upload.staging.mapped[cur] = false;
    const WGPUBuffer stg_buf = _sg.wgpu.upload.staging.buf[cur];
    SOKOL_ASSERT(stg_buf);
    sg_wgpu_js_buffer_unmap_range(stg_buf);
    wgpuBufferUnmap(stg_buf);
}
...
_SOKOL_PRIVATE WGPUBindGroup _sg_wgpu_create_images_bindgroup(WGPUBindGroupLayout bgl, _sg_image_t** imgs, uint32_t num_imgs) {
    SOKOL_ASSERT(_sg.wgpu.dev);
    SOKOL_ASSERT(num_imgs <= _SG_WGPU_MAX_SHADERSTAGE_IMAGES);

    WGPUBindGroupEntry img_bgentries[_SG_WGPU_MAX_SHADERSTAGE_IMAGES * 2];
    memset(&img_bgentries, 0, sizeof(img_bgentries));
    for (uint32_t img_index = 0; img_index < num_imgs; img_index++) {
        const uint32_t tex_binding = img_index * 2 + 0;
        const uint32_t smp_binding = img_index * 2 + 1;
        WGPUBindGroupEntry* tex_entry = &img_bgentries[tex_binding];
        WGPUBindGroupEntry* smp_entry = &img_bgentries[smp_binding];
        tex_entry->binding = tex_binding;
        tex_entry->textureView = imgs[img_index]->wgpu.tex_view;
        smp_entry->binding = smp_binding;
        smp_entry->sampler = imgs[img_index]->wgpu.sampler;
    }
    WGPUBindGroupDescriptor bg_desc;
    memset(&bg_desc, 0, sizeof(bg_desc));
    bg_desc.layout = bgl;
    bg_desc.entryCount = 2 * num_imgs;
    bg_desc.entries = &img_bgentries[0];
    ...
    _sg_image_t** fs_imgs, int num_fs_imgs)
{
    ...
    if (ib) {
        ...
        SOKOL_ASSERT((uint64_t)ib->cmn.size > offset);
        const uint64_t max_bytes = (uint64_t)ib->cmn.size - offset;
    ...
    for (uint32_t slot = 0; slot < (uint32_t)num_vbs; slot++) {
        ...
        SOKOL_ASSERT((uint64_t)vbs[slot]->cmn.size > offset);
        const uint64_t max_bytes = (uint64_t)vbs[slot]->cmn.size - offset;
    ...
    // FIXME: check performance with a hashed bindgroup cache
    if (num_vs_imgs > 0) {
        ...
        SOKOL_ASSERT(vs_bgl);
        WGPUBindGroup vs_img_bg = _sg_wgpu_create_images_bindgroup(vs_bgl, vs_imgs, (uint32_t)num_vs_imgs);
    ...
    if (num_fs_imgs > 0) {
        ...
        SOKOL_ASSERT(fs_bgl);
        WGPUBindGroup fs_img_bg = _sg_wgpu_create_images_bindgroup(fs_bgl, fs_imgs, (uint32_t)num_fs_imgs);
...
_SOKOL_PRIVATE void _sg_wgpu_apply_uniforms(sg_shader_stage stage_index, int ub_index, const sg_range* data) {
    ...
    SOKOL_ASSERT(_sg.wgpu.ub.staging.mapped[_sg.wgpu.ub.staging.cur]);

    const WGPUBuffer ub_buf = _sg.wgpu.ub.staging.buf[_sg.wgpu.ub.staging.cur];
    SOKOL_ASSERT(ub_buf);
    sg_wgpu_js_buffer_copy_range(ub_buf, (int)_sg.wgpu.ub.offset, data->ptr, (int)data->size);
